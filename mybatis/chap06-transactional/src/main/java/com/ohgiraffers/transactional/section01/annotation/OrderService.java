...
public class OrderService {
    ...
    /* 전파행위 옵션 propagation
     *  REQUIRED : 진행 중인 트랜잭션이 있으면 현재 메소드를 그 트랜잭션에서 실행하되 그렇지 않은 경우 새 트랜잭션을 시작해서 실행한다. -> default option
    ...
    /* 격리레벨 (동시성) : DBMS 따라 기본값 달라진다.
     *  DEFAULT : DB의 기본 격리 수준을 사용한다. 대다수는 READ_COMMITTED가 기본 격리 수준이다.
     *  READ_UNCOMMITTED : 다른 트랜젝션이 아직 커밋하지 않은 값을 다른 트랜젝션이 읽을 수 있다. // 격리레벨 낮다, 아래 항목일수록 높아진다.
     ...
     *                 동시성 문제는 모두 해소되지만 성능은 현저하게 떨어진다. (모든 동시성 문제는 막을 수 있다.) // 격리레벨 높다
     * 설명.
    ...
    /* Service 계층부터 개발 할 경우 사용자 입력 값이 어떻게 DTO or Map 으로 묶여서
    * Controller 계층에서 넘어오는지를 충분히 고민 한 수 매개변수를 작성한다. */
    @Transactional(isolation = Isolation.DEFAULT, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    // ㄴ 메소드 내부에서 exception 발생 시 rollback, 없을 시 commit 기능이 AOP로 구현되어 있다.
    public void registNewOrder(OrderDTO orderDTO){
        ...
        // 엔터티 : 데이터베이스 테이블과 온전히 일지, DTO : 데이터 옮기는 데 목적이므로 기능에 따라간다.

        /* 2. 주문한 메뉴 별로 Menu 엔터티에 담아서 조회(select) 해오기 - 부가적인 메뉴의 정보 추출 (단가 등) */
        // memuCodes값을 가져와 메뉴 목록 조회하는 것을 요청
        List<Menu> menus = menuMapper.selectMenuByMenuCodes(map);
        menus.forEach(System.out::println);

        /* 3. 해당 주문건의 총 합계를 계산 */
        int totalOrderPrice = calTotalOrderPrice(orderDTO.getOrderMenus(), menus);
        ...
        for(OrderMenuDTO orderMenuDTO : orderDTO.getOrderMenus()){
            /* OrderMenuDTO(dte) -> OrderMenu(entity) 로 전달 */
